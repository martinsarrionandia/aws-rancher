...
#sudo docker run -e CATTLE_BOOTSTRAP_PASSWORD="${bootstrap_password}" -d --restart=unless-stopped \
#-p "${rancher_ip}":80:"${emissary_node_port_http}" \
#-p "${rancher_ip}":443:"${emissary_node_port_https}" \
#-p "${rancher_ip}":"${rancher_admin_http}":80 \
#-p "${rancher_ip}":"${rancher_admin_https}":443 \
